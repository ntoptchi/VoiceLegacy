  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok || !payload.success || !payload.userId) {
        throw new Error(
          payload.error ?? `Failed to create profile (status ${response.status}).`,
        );
      }

      const pendingVoiceId = sessionStorage.getItem("pendingVoiceId");
      if (pendingVoiceId) {
        await fetch("/api/voice/claim", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ voiceId: pendingVoiceId }),
        });
        sessionStorage.removeItem("pendingVoiceId");
      }
  ...
  return (
    ...
    >
      <header className="flex flex-col gap-sm text-center">
        ...
        >
          Almost there — protect your voice
        </h1>
        <p
